...
- [x] Progression pour gros fichiers (`-progress pipe:1`) — câblé (2026-07-08) : `clean_video` streame l'avancement du remux par un `Channel<CleanEvent>` ajouté à `clean_files` (octets écrits `total_size` ÷ taille d'entrée via `remux_percent`, borné à 99 — pas de sonde de durée) ; `FileCard` affiche la barre réelle pendant le lavage vidéo (les images, instantanées, n'en émettent pas ; le lavage post-téléchargement passe un no-op). Vérifié : ffmpeg émet bien des `total_size` périodiques et croissants (11 relevés sur un remux pacé `-re`), `remux_percent` unit-testé, un remux réel de 168 Mo passe 0→99→propre
...
- [x] Notification système en fin de téléchargement long — ajouté (2026-07-08, dépendance validée) : `tauri-plugin-notification` + `@tauri-apps/plugin-notification` + `notification:default` dans la capability + plugin enregistré dans `lib.rs`. Se déclenche à l'événement `completed` **uniquement si l'app n'a pas le focus** (`document.hasFocus()` — le seul moment où c'est utile) ; permission demandée à la première fois. `notifyDownloadDone` dans `recuperer.ts`. Build de prod vert avec le plugin (`notify-rust` compilé, doctor toujours 4/4)
...
- [x] Build NSIS de production vert : `lavoir_1.0.0_x64-setup.exe` (78,2 Mio), `--doctor` release **4/4** (yt-dlp 2026.07.04, ffmpeg/ffprobe 8.1.2, exiftool 13.59), sidecars + `exiftool_files` bien à côté de l'exe release. Version passée de 0.1.0 à **1.0.0** (tauri.conf.json / package.json(+lock) / Cargo.toml(+lock)) le 2026-07-08 ; re-vérifiée verte (svelte-check 0/0, cargo test 10/10, doctor release 4/4)
